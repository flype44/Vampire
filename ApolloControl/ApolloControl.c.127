...
ULONG Help(void)
{
	...
	/*COLOR2*/ "ZS=ZOOMSHIFT/N    " /*COLOR7*/ "Change the Zoom SHIFT value. 0 to 65535, for V4(+)\n"
...
/**********************************************************
 ** 
 ** SetZoomShift(ULONG mode)
 ** 
 **********************************************************/

ULONG SetZoomShift(ULONG shift)
{
	ULONG result = RETURN_WARN;
	
	if(!IsV2())
	{
		printf("ZoomShift: %04X", shift);
	...
	if(DOSBase)
	{
		...
		else
		{
			...
			if(rdargs = (struct RDArgs *)ReadArgs(TEMPLATE, opts, NULL))
			{
				...
				if(opts[OPT_ZOOMSHIFT])
					result = SetZoomShift(*(LONG *)opts[OPT_ZOOMSHIFT]);
